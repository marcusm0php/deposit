...
class EpayHandler
{
    ...
    public static function VerifyMac($param_array, $commkey = null, $cert = null) {
        ...
        if(array_key_exists('sign_type', $param_array) && $param_array['sign_type'] === 'RSA') {
            ...
            $mac = EpayHandler::Signature($param_array, $commkey);
    ...
    public function epAuth($trac_no, $acct_type, $bank_no, $card_no, $user_name = null, $cert_no = null, $card_phone = null, $expireDate = null, $cvn = null) {
        ...
        if($this -> epay_config['isDevEnv'])
            return $this -> redirectService(EpayHandler::EP_DEV_API, $param_array);
        else
            return $this -> redirectService(EpayHandler::EP_PROD_API, $param_array);
    }
    ...
    public function epAuthSyncWithSms($trac_no, $acct_type, $bank_no, $card_no, $user_name, $cert_no, $card_phone, $expireDate = null, $cvn = null) {
        ...
        if($this -> epay_config['isDevEnv'])
            return $this -> postService(EpayHandler::EP_DEV_API, $param_array, null);
        else
            return $this -> postService(EpayHandler::EP_PROD_API, $param_array, null);
    }
    ...
    public function gpPay($order_no, $order_amount, $order_title, $order_desc, $remote_ip) {
        ...
        if($this -> epay_config['isDevEnv'])
            return $this -> redirectService(EpayHandler::GP_DEV_API, $param_array);
        else
            return $this -> redirectService(EpayHandler::GP_PROD_API, $param_array);
    }
    ...
    public function gpQuery($order_no, $order_date = null) {
        ...
        if($this -> epay_config['isDevEnv'])
            return $this -> postService(EpayHandler::GP_DEV_API, $param_array, null);
        else
            return $this -> postService(EpayHandler::GP_PROD_API, $param_array, null);
    }
    ...
    public function dlSettleFile($rcpt_type, $trans_date, $save_file_name) {
        ...
        if($rcpt_type === '6' || $rcpt_type === '7' || $rcpt_type === '8') {
            ...
            if($this -> epay_config['isDevEnv'])
                $response = $this -> postService(EpayHandler::PY_DEV_API, $param_array, $save_file_name);
            else
                $response = $this -> postService(EpayHandler::PY_PROD_API, $param_array, $save_file_name);
        } else {
            $param_array['rcpt_type']	= $rcpt_type;
            $param_array['service']		= 'cib.epay.acquire.settleFile';
            if($this -> epay_config['isDevEnv'])
                $response = $this -> postService(EpayHandler::GP_DEV_API, $param_array, $save_file_name);
            else
